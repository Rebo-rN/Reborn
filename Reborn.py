...
print('\033[92;1m>>\033[1;37m Checking modules...')
...
#---------- NEPAL Number Generator ✓---------------
   
def get_np_phone():
    prefixes = ['98', '97']
    prefix = random.choice(prefixes)
    number = ''.join(random.choices(string.digits, k=8))
    return f'+977{prefix}{number}'
...
{G}   _____      _                       
  ...
  |  _  // _ \\ '_ \\ / _ \\| '__| '_ \\  
  | | \\ \\  __/ |_) | (_) | |  | | | | 
  |_|  \\_\\___|_.__/ \\___/|_|  |_| |_|   {Y}XD{w}
{m}============================================={w}
{Y}[+] Owner{w}    : {G}Dipak{w}         {Y}Version{R} : {w}0.1
{Y}[+] Facebook{w} : {G}Reborn Bëb{w}
{Y}[+] Status{w}   : {G}{random_status}{w}
{m}============================================={w}
"""
...
def createfb_method_3():
    ...
    for i in range(num):
        try:
            ...
            firstname, lastname = get_np_phone()
            ...
            if "c_user" in login_coki:
                ...
                if status == "live":
                    ...
                    if show_details == 'y':
                        print(f"\r{V}[{G}•{V}] Name   : {G}{firstname} {lastname}{W}")
                        print(f"\r{V}[{G}•{V}] Number : {G}{phone}{W}")                                               
                        print(f"\r{V}[{G}•{V}] UID    : {G}{uid}{W}")
                        print(f"\r{V}[{G}•{V}] PASS   : {G}{pww}{W}")
                        print(f"\r{G}{coki}{W}")
                        print(f"{V}─────────────────────────────────────────────{W}")
                    else:
                        print(f"\r{G} Auto FB{W}-{G}[OK] {uid} | {pww}")
                    try:
...
def createfb_method_2():
    ...
    for i in range(num):
        try:
            ...
            response = ses.get("https://x.facebook.com/reg")
            form = extractor(response.text)
            firstname, lastname = get_np_name()
...
def createfb_method_1():
    ...
    num = int(input(f"{W}[{G}•{W}]{G} HOW MANY ACCOUNT {W}:{G} "))
    linex()
    print(f"{W}[{G}1{W}]{G} CUSTOM PASSWORD")
    print(f"{W}[{G}2{W}]{G} AUTO PASSWORD")
    linex()
    password_choice = input(f"{W}[{G}•{W}]{G} CHOISE {W}:{G} ")
    pww = get_pass() if password_choice == '2' else input(f"{W}[{G}•{W}]{G} ENTER PASSWORD {W}:{G} ")
    linex()
        ...
        try:
            ...
            if "c_user" in login_coki:
                ...
                if show_details == 'y':
                    ...
                    print(f"\r{V}Auto FB{W}-{G}[OK] {uid} | {pww}")
                with open('/sdcard/Auto_Creat.txt', 'a') as f:
                    f.write(f"{uid}|{pww}|{coki}\n")
                oks.append(uid)
            elif "checkpoint" in login_coki:
